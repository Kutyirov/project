...
void delete_noise(Mat &img, unsigned coef) {
  ...
  for (int i = 0; i < img.rows; i++) {
    for (int j = 0; j < img.cols; j++) {
      if ((int(img.at<cv::Vec3b>(i, j)[0]) > coef) ||
          (int(img.at<cv::Vec3b>(i, j)[1]) > coef) ||
          (int(img.at<cv::Vec3b>(i, j)[2]) > coef)) {
...
void find_elements(int **&work, unsigned x1, unsigned y1, unsigned x2,
                   unsigned y2, unsigned speed_coef,
        ...
        for (unsigned j = 0; j < x3; j++) {
          ...
          if (quanity > cap.rows * cap.cols - 400)
            for (unsigned i1 = i; i1 < cap.rows + i; i1++)
              for (unsigned j1 = j; j1 < cap.cols + j; j1++) work[i1][j1] = t;
        }
      }
    }
  }
...
void find_wire(int **&work, unsigned x1, unsigned y1, unsigned x2,
               unsigned y2) {
    ...
    for (unsigned j = 0; j < x2 - x1 + 61; j++)
      ...
      if (work[i][j] == 255) {  //ищем крайние точки
        ...
        while (!((work[y1][x1] > 0) && (work[y1][x1] < 19))) {
          ...
          std::cout << y1 << " " << x1 << " " << direction1 << " " << number
                    << " " << work[y1][x1] << std::endl;
        }
        std::cout << "second\n";
        while (!((work[y2][x2] > 0) && (work[y2][x2] < 19))) {
          ...
          std::cout << y2 << " " << x2 << " " << direction2 << " " << number
                    << " " << work[y2][x2] << std::endl;
        }
        unsigned l1 = 0;
        unsigned m1 = 0;
        ...
        if (work[y1][x1] % 2 == 1) {
          l1 = x1;
          m1 = x1;
          while (work[y1][l1] != 0) l1 = l1 - 1;
          while (work[y1][m1] != 0) m1 = m1 + 1;
          x1 = (l1 + m1) / 2;
        ...
        if (work[y2][x2] % 2 == 1) {
          l2 = x2;
          m2 = x2;
          while (work[y2][l2] != 0) l2 = l2 - 1;
          while (work[y2][m2] != 0) m2 = m2 + 1;
          x2 = (l2 + m2) / 2;
        } else if (work[y2][x2] % 2 == 0) {
          l2 = y2;
          m2 = y2;
          while (work[l2][x2] != 0) l2 = l2 - 1;
          while (work[m2][x2] != 0) m2 = m2 + 1;
          y2 = (l2 + m2) / 2;
        }

        std::cout << std::endl
                  << y1 << " " << x1 << " " << y2 << " " << x2 << std::endl;
        ...
        if ((y1 + 5 > y2 - 5) && (y1 - 5 < y2 + 5)) {
          ...
          for (unsigned l = y1 - 2; l < y1 + 3; l++)
            for (unsigned m = x1 + 1; m < x1 + 10; m++) work[l][m] = 254;
          if (y2 > y1) {
            for (unsigned l = y1 - 2; l < y2 + 3; l++)
              for (unsigned m = x1 + 10; m < x1 + 15; m++) work[l][m] = 254;
            for (unsigned l = y2 - 2; l < y2 + 3; l++)
              for (unsigned m = x1 + 15; m < x2; m++) work[l][m] = 254;
          } else {
          ...
          for (unsigned l = x1 - 2; l < x1 + 3; l++)
            for (unsigned m = y1 + 1; m < y2; m++) work[m][l] = 254;
        } else {  //углы
          if (x1 > x2) {
            swap(x1, x2);
            swap(y1, y2);
          ...
          if ((work[y1 - 1][x1] == 0) && (work[y2][x2 - 1] == 0)) {
            for (unsigned l = y2 - 2; l < y1; l++)
              for (unsigned m = x1 - 2; m < x1 + 3; m++) work[l][m] = 254;
            for (unsigned l = y2 - 2; l < y2 + 3; l++)
              for (unsigned m = x1 + 3; m < x2; m++) work[l][m] = 254;
          }
          if ((work[y1][x1 + 1] == 0) && (work[y2 - 1][x2] == 0)) {
            for (unsigned l = y1 - 2; l < y2; l++)
              for (unsigned m = x2 - 2; m < x2 + 3; m++) work[l][m] = 254;
            for (unsigned l = y1 - 2; l < y1 + 3; l++)
              for (unsigned m = x1 + 1; m < x2 - 2; m++) work[l][m] = 254;
          }
          if ((work[y1][x1 + 1] == 0) && (work[y2 + 1][x2] == 0)) {
...
void push_elements(int **&work, unsigned x1, unsigned y1, unsigned x2,
                   unsigned y2) {
  Mat el;
  ...
  for (unsigned i = 0; i < y2 - y1 + 61; i++)
    for (unsigned j = 0; j < x2 - x1 + 61; j++) {
      if ((work[i][j] > 0) && (work[i][j] < 19)) {
        ...
        unsigned y5 = i;
        unsigned x5 = j;
        while (work[y5 + 1][j] != 0) y5++;
        while (work[i][x5 + 1] != 0) x5++;
        // std::cout<< i << " " << x5 <<std::endl << y5 << " " << j << std ::
        // endl << "next\n";
        unsigned dir = work[y5][x5] % 2;
        for (unsigned i1 = i; i1 <= y5; i1++)
          for (unsigned j1 = j; j1 <= x5; j1++) work[i1][j1] = 0;
        y5 = (i + y5) / 2;
        x5 = (j + x5) / 2;
        // unsigned dir = work[y5][x5] % 2;
        ...
        for (unsigned i1 = y5 - el.rows / 2; i1 < y5 + el.rows / 2 - 5; i1++)
          for (unsigned j1 = x5 - el.cols / 2; j1 < x5 + el.cols / 2 - 5; j1++)
            work[i1][j1] = el.at<cv::Vec3b>(i1 - y5 + el.rows / 2,
                                            j1 - x5 + el.cols / 2)[0];
        ...
        if (dir == 1) {  //вертикальные провода
          while (work[y6][x6] == 0) y6++;
          y6 += 7;
          while (work[y7][x7] == 0) y7--;
          y7 -= 7;
          unsigned x8 = x6;
          unsigned x9 = x7;
          unsigned y8 = y6;
          ...
          y6 -= 4;
          y7 += 4;
          while (work[y8][x8] == 0) y8++;
          while (work[y9][x9] == 0) y9--;
          for (unsigned i1 = y6; i1 < y8; i1++)
            for (unsigned j1 = x6 - 2; j1 < x6 + 3; j1++) work[i1][j1] = 255;
          for (unsigned i1 = y9; i1 < y7; i1++)
            for (unsigned j1 = x7 - 2; j1 < x7 + 3; j1++) work[i1][j1] = 255;
          // std::cout << "y9 = " << y9 << " y7 = " << y7 << " y6 = " << y6 << "
          // y8 = " << y8 << std::endl;
        } else {
          while (work[y6][x6] == 0) x6++;
          x6 += 7;
          while (work[y7][x7] == 0) x7--;
          x7 -= 7;
          unsigned x8 = x6;
          unsigned x9 = x7;
          unsigned y8 = y6;
          ...
          x6 -= 4;
          x7 += 4;
          while (work[y8][x8] == 0) x8++;
          while (work[y9][x9] == 0) x9--;
          for (unsigned i1 = x6; i1 < x8; i1++)
            for (unsigned j1 = y6 - 2; j1 < y6 + 3; j1++) work[j1][i1] = 255;
          for (unsigned i1 = x9; i1 < x7; i1++)
            for (unsigned j1 = y7 - 2; j1 < y7 + 3; j1++) work[j1][i1] = 255;
        }
      }
    }
}
